...
adminQQ: ['10001', '10002']       # ← 必改：管理员 QQ 号白名单
superAdminQQ: ['10001']           # ← 超级管理员，可在群内 /添加管理员
atQQ: false                       # 是否在回复中 @用户
debug: false                      # 调试日志
```

**管理权限**：
- `adminQQ` 与群内 `/添加管理员` 添加的账号可执行：`/开始抢答`、`/结束抢答`、`/结束本题` 等
- `superAdminQQ` 额外可执行 `/添加管理员`、`/移除管理员`
- 非管理员执行这些指令会收到「你没有权限执行此操作」
- 普通用户（注册后）可正常 `/回答` 答题
